...
import { Card, CardHeader, CardBody, CheckItem, TradeCard, Alert, SubTabs } from '../ui/index'
import { toast } from '../ui/index'
import { addTrade, updateTrade, getTrades, getAccounts, getActiveAcc, todayStr, deleteTrade } from '../../lib/db'
import {
        ...
        <>
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              {/* SMC Details */}
              <div className="border border-border rounded-lg p-3 space-y-3">
                <div className="text-[9px] font-mono font-semibold uppercase tracking-widest text-cyan">SMC Confluence</div>
                    ...
                    return (
                      <button key={id} onClick={() => set(id, !form[id])}
                        className={`px-2.5 py-1 rounded text-[10px] font-mono transition-colors ${
                          active ? 'bg-emerald/15 text-emerald border border-emerald/25' : 'text-ink-4 border border-border hover:border-border-3'
                        }`}>{label}</button>
                    )
                  })}
                </div>
